  ...
  _render(){
    ...
    return (
      <View style={styles.listItem}>
        {icon?(<Icon name={icon} size={px2dp(iconSize||20)} style={{width: 22, marginRight:5, textAlign:"center"}} color={color || "#8B8989"} />):null}
        <View style={[styles.listInfo, {borderTopWidth: !first?1:0}]}>
          <View style={{flex: 1}}><Text style={{fontSize: 15}}>{name}</Text></View>
          <View style={styles.listInfoRight}>
            {subName?(<Text style={{color: "#aaa", fontSize:15}}>{subName}</Text>):null}
            {avatar?(<Image source={avatar} style={{width: 40, height: 40, resizeMode: "cover", overflow:"hidden", borderRadius: 18}}/>):null}
